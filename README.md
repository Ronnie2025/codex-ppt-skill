...
它不是一个一键万能 PPT SaaS，而是一套给 Codex 使用的工作流和工具箱。重点是把 PPT 生成、图片页封装、语义资产重组、SVG 结构重绘这些路径分清楚，让用户知道什么时候该追求视觉效果，什么时候该追求可编辑性。
...
三条路径不是只能三选一。路径 A 可以先把材料做成高质量图片型 PPT，等视觉方向确认后，再把 A 生成的每页图片交给路径 B 做语义资产重组，或交给路径 C 做 SVG 结构重绘。默认优先考虑 B，因为它更接近用户后续维护 PPT 的真实需求；只有明确只要 SVG 或要低成本结构样板时，再选 C。
...
| 维度 | 语义资产重组 | SVG 结构重绘 |
|---|---|---|
| 适合输入 | 原始截图、图片页、路径 A 生成的单页 PNG | 原始截图、图片页、路径 A 生成的单页 PNG |
| 时间成本 | 较高，需要 inventory、资产生成、切图、PPT 组装和验证 | 较低，通常适合先做结构样板或单页快速验证 |
| token 消耗 | 高，需要多轮识别、资产提示词、布局复刻和 QA 解释 | 中低，主要消耗在页面理解和 SVG 代码生成 |
| 视觉效果 | 更适合保留图标、箭头、3D 装饰和复杂视觉层次 | 对线框、流程图、信息图较稳；复杂 3D、插画、拟物元素会被简化 |
| 可编辑性 | PPT 内文字、容器、形状和语义资产可分别选中、移动、替换 | SVG 内部可编辑，但导入 PowerPoint 后不保证对象级稳定可编辑 |
| 适合选择 | 明确要可编辑 PPTX，且愿意用更多时间和 token 换维护性 | 只要 SVG、网页/文档复用、轻量结构还原、低成本试探 |
...
6. 若后续需要可编辑 PPTX 或 SVG，把最终页图作为路径 B/C 的参考输入。
...
- 先判断交付物类型，再开始生成。图片型 PPT、可编辑 PPTX、SVG 是三种不同承诺。
- 不把图片型 PPT 说成可编辑 PPT。
