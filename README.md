...
![项目流程图](./assets/workflow/codex-ppt-skill-flow-v2.svg)

## 使用顺序

先问一个问题：**后续需不需要编辑？**

- 不需要编辑：直接走路径 A，生成图片型 PPT，交付 PPTX / PDF / PNG 就够了。
- 需要编辑：把原始截图、图片页或路径 A 生成的页图作为输入，再选一种拆解方式。
- 默认优先路径 B 元素重组；只有明确只要 SVG、网页/文档复用，或想先低成本看结构时，再选路径 C。

| 路径 | 适合目标 | 最终交付 | 关键边界 |
|---|---|---|---|
| 整页生图 PPT | 方案汇报、售前材料、培训讲解、复盘展示，优先看起来好 | PPTX / PDF / PNG / 汇总预览图 / 逐页提示词 | 正文、图表和版式在图片里，不承诺全元素可编辑 |
| 元素重组 | 要把图片页重建为实用级可编辑 PPTX，并保留图标、箭头、3D 装饰等美化元素 | 可编辑 PPTX / asset_manifest / 验证报告 / 渲染差异图 | 原图只做参考，不能把整页原图或原图硬裁片当最终素材 |
| SVG 拆解 | 用户只想把图片页拆成 SVG，或用于轻量矢量结构还原 | SVG / SVG 预览 / 可选 PPTX 嵌入 | SVG 导入 PowerPoint 后未必保持内部对象稳定可编辑，不能当成完整 PPT 反编译 |
...
| 方式 | 什么时候选 | 时间与 token | 结果边界 |
|---|---|---|---|
| 元素重组 | 要可编辑 PPTX，并保留图标、箭头、3D 装饰和视觉层次 | 更高 | PPT 内文字、形状和独立资产可分别选中、移动、替换 |
| SVG 拆解 | 只要 SVG、文档/网页复用，或先快速还原结构 | 更低 | SVG 适合结构复用，但导入 PPT 后不保证对象级稳定可编辑 |

### 拆解效果

| 元素重组：面向可编辑 PPTX | SVG 拆解：面向轻量结构复用 |
|---|---|
| ![元素重组效果](./assets/examples/decompose-element-rebuild.svg) | ![SVG 拆解效果](./assets/examples/decompose-svg-redraw.svg) |
