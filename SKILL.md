...
description: "用于 PPT 视觉生成与图片页可编辑重组的工作流：支持用 imagegen 生成整页图片型 PPT，也支持把图片版 PPT/截图页按语义资产拆解并重建为可编辑 PPTX。"
---
...
先判断用户真实目标，再选择模式。不要把图片型 PPT 说成可编辑 PPT；也不要把整页截图当成可编辑重建的最终交付。
...
不适配这两种模式时，直接说明原因，并建议使用文档、数据报表、原生可编辑 PPT 或设计工具流程。
...
7. 渲染 PPTX 为 PNG/PDF，使用 `scripts/compare_render.py` 生成 contact sheet 和 diff heatmap。
8. 使用 `scripts/validate_semantic_deck.py` 检查没有整页原图、没有 reference hash、没有 raw crop 媒体，并输出验证报告。
