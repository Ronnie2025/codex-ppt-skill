...
- `provided_asset` 只适合用户已提供的合规素材或 synthetic demo；不能把程序化图标、通用图标库或原图硬裁片当作生产级视觉复刻资产。
...
4. 为资产写 `prompts/assets_cycle_*.jsonl` 或 prompt pack，用 Codex `imagegen` 生成独立资产或 isolated asset grid。小图标可以成组生成；中心大视觉和复杂元素必须单独生成。要求无文字、无数字、无标签、无卡片框、无背景碎片。
